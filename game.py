...
class Game:
    ...
    def gameLoop(self):
        ...
        while True:
            ...
            self.drawEverything()
            pg.display.update()
            self.clock.tick(60) #limited to 60 fps
    ...
    def updateEverything(self,dt):
        if self.is_enter_pressed:
            #moving the ground
            self.ground1_rect.x-=int(self.move_speed*dt)#movement speed
            self.ground2_rect.x-=int(self.move_speed*dt)

            if self.ground1_rect.right<0:
                self.ground1_rect.x=self.ground2_rect.right#if ground1 reach out of display then make ground 1 at the the same cordiante as ground 2
